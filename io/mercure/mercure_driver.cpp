...
MercureDriver::MercureDriver()
    : status_(GX_STATUS_SUCCESS), device_(nullptr), pFrameBuffer_(nullptr) {
    init_sdk();
    LoadParam();
...
void MercureDriver::read(cv::Mat & img, std::chrono::steady_clock::time_point & timestamp) {
    if (img.empty()) {
        MercureDriver::CreateMat(img);
    }
    *this >> img;
}
...
GX_STATUS MercureDriver::init_sdk() {
    ...
    status_ =
        GXSetInt(device_, GX_DS_INT_STREAM_TRANSFER_SIZE, ACQ_TRANSFER_SIZE);
    status_ = GXSetInt(device_, GX_DS_INT_STREAM_TRANSFER_NUMBER_URB,
...
void MercureDriver::LoadParam(const std::string& file_name) {
    ...
    if (!fs.isOpened()) {
        std::cout << " Camrea WRONG" << std::endl;
    }

    fs["exposure_auto"] >> param_.exp_auto_;
    fs["exposure_time"] >> param_.exp_time_;
    if (param_.exp_auto_) {
        status_ = GXSetEnum(device_, GX_ENUM_EXPOSURE_AUTO,
        ...
        status_ = GXSetFloat(device_, GX_FLOAT_AUTO_EXPOSURE_TIME_MIN,
                             param_.exp_time_ - 200);
        status_ = GXSetFloat(device_, GX_FLOAT_AUTO_EXPOSURE_TIME_MAX,
                             param_.exp_time_ + 200);
    ...
    if (param_.w_auto_) {
        ...
        status_ = GXSetEnum(device_, GX_ENUM_BALANCE_RATIO_SELECTOR,
...
void MercureDriver::GetVision() {
    ...
    GXGetString(device_, GX_STRING_DEVICE_SERIAL_NUMBER, pszSerialNumber,
                &nSize);
...
void MercureDriver::operator>>(cv::Mat& Image) {
    ...
    try {
        if (pFrameBuffer_ == nullptr) {
            std::cerr << "Camera error. Is the camera online?" << std::endl;
          Image = cv::Mat();
        }
        
        DxRaw8toRGB24(
            pFrameBuffer_->pImgBuf, rgbImagebuf_, pFrameBuffer_->nWidth,
            pFrameBuffer_->nHeight, RAW2RGB_NEIGHBOUR,
            DX_PIXEL_COLOR_FILTER(4), // DX_PIXEL_COLOR_FILTER(colorfilter_),
            false);

        if (Image.empty()) {
            std::cerr << "The Image Mat is empty; please initialize it in advance." << std::endl;
        }

        memcpy(Image.data, rgbImagebuf_,
               pFrameBuffer_->nHeight * pFrameBuffer_->nWidth * 3);
...
void MercureDriver::operator>>(Mat_time &Image)
{
    ...
    try
    {
        DxRaw8toRGB24(pFrameBuffer_->pImgBuf,
                      ...
                      false);
        memcpy(Image.data, rgbImagebuf_, pFrameBuffer_->nHeight *
pFrameBuffer_->nWidth * 3);
...
void MercureDriver::CreateMat(cv::Mat& mat_image) {
    mat_image.create(ACQ_FRAME_HEIGHT, ACQ_FRAME_WIDTH, CV_8UC3);
}
